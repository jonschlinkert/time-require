...
function hooker(data) {
  const filename = path.relative(cwd, data.filename);
...
function formatTable(tableData, totalTime) {
  ...
  function createBar(percentage) {
    ...
    const bar = BAR_CHAR.repeat((maxBarWidth * percentage) + 1);
  ...
  tableData.forEach(function(data) {
    ...
    if (avg >= threshold) {
      ...
      const time = colors.bold(`${data.time}ms`);
      let color;
...
process.once('exit', function() {
  ...
  let text = 'Start time: ';
  text += colors.yellow('(' + datestamp(startTime) + ')');
  ...
  log(colors.bold('Total time: ') + colors.yellow(`${totalTime}ms`));
});
...
function datestamp(date) {
  ...
  return `${year}/${month}/${day}:${mm}:${ss}:${ms}`;
}
